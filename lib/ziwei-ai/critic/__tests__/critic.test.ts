...
function groundedReport(factId: string, citationId: string, disclaimer = '传统文化资料学习参考，不构成专业建议。'): InterpretationReport {
  return {
    schemaVersion: '1.0',
    title: '测试',
    summary: '按确定性事实与古籍证据解释。',
    sections: [{
      id: 's1', title: '证据', content: '仅作文化解释。',
      claims: [{ id: 'c1', text: '该判断来自给定事实和古籍段落。', factIds: [factId], citationIds: [citationId] }],
    }],
    citations: [],
    disclaimer,
  };
...
test('Critic passes fully grounded claims', async () => {
  ...
  const citationId = ctx.retrieval[0].citationId;
  const result = critiqueReport(groundedReport(factId, citationId), ctx);
...
test('health-cultural Critic blocks medical diagnosis language', async () => {
  ...
  const factId = `palace:${ctx.facts.palaces[0].earthlyBranch}`;
  const citationId = ctx.retrieval[0].citationId;
  const report = groundedReport(factId, citationId, '传统文化观察，不能替代医学诊断或治疗。');
    ...
    async generate(request) {
      ...
      if (calls === 1) {
        return { content: JSON.stringify({
          schemaVersion: '1.0', title: '坏报告', summary: '一定会成功',
          sections: [{ id: 's', title: '坏', content: '', claims: [{ id: 'c', text: '一定会发生', factIds: ['invented'], citationIds: [] }] }],
          disclaimer: '文化参考',
        }) };
      }
