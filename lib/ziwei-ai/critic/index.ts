...
export function critiqueReport(report: InterpretationReport, context: AgentContext): CriticResult {
  ...
  for (const claim of claims) {
    ...
    if (invalidFacts.length) issues.push({ severity: 'error', code: 'unknown-fact-id', claimId: claim.id, message: `未知事实引用：${invalidFacts.join(', ')}` });
    if (invalidCitations.length) issues.push({ severity: 'error', code: 'unknown-citation-id', claimId: claim.id, message: `未知古籍引用：${invalidCitations.join(', ')}` });
    ...
    if (FATALISTIC.test(claim.text)) issues.push({ severity: 'error', code: 'fatalistic-language', claimId: claim.id, message: '检测到宿命化或绝对化断言。' });
  ...
  if (FATALISTIC.test(fullText)) issues.push({ severity: 'error', code: 'fatalistic-report', message: '报告正文含宿命化绝对断言。' });
  if (context.topic === 'health-cultural') {
    if (MEDICAL_DIAGNOSIS.test(fullText)) issues.push({ severity: 'error', code: 'medical-report', message: '报告正文越过传统文化边界进入医学诊断/治疗。' });
  ...
  return { passed: errorCount === 0 && groundedClaimRatio >= 0.75, score, groundedClaimRatio, citationReferencePrecision, issues };
}

function revisionMessages(result: InterpretChartResult, critic: CriticResult) {
  return [
    ...buildAgentMessages(result.context),
    { role: 'assistant' as const, content: JSON.stringify(result.report) },
    { role: 'user' as const, content: `Critic 发现以下问题。只修正这些问题，仍严格输出原 JSON schema，不得增加无证据事实：\n${JSON.stringify(critic.issues)}` },
  ];
}

export async function interpretWithCritic(request: InterpretChartRequest, provider: ModelProvider, reviseOnce = true): Promise<CritiquedInterpretationResult> {
...
export interface EvaluationCaseResult { name: string; critic: CriticResult }
